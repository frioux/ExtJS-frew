...
(function() {
    ...
    // private
    $D.parseFunctions = {count:0};
    // private
    $D.parseRegexes = [];
    // private
    $D.formatFunctions = {count:0};

    // private
    $P.dateFormat = function(format) {
        if ($D.formatFunctions[format] == null) {
            $D.createNewFormat(format);
        }
        var func = $D.formatFunctions[format];
        return this[func]();
    };
     ...
     * @method
     */
    $P.format = $P.dateFormat;

    // private
    $D.createNewFormat = function(format) {
        var funcName = "format" + $D.formatFunctions.count++;
        $D.formatFunctions[format] = funcName;
    ...
    $D.formatCodes = {
        ...
        c: function() { // ISO-8601 -- GMT format
            for (var c = "Y-m-dTH:i:sP", code = [], i = 0, l = c.length; i < l; ++i) {
                var e = c.charAt(i);
                code.push(e == "T" ? "'T'" : $D.getFormatCode(e)); // treat T as a character literal
            }
            return code.join(" + ");
        },
        /*
    ...
    // private
    $D.getFormatCode = function(character) {
        var f = $D.formatCodes[character];

        if (f) {
          f = Ext.type(f) == 'function'? f() : f;
          $D.formatCodes[character] = f; // reassign function result to prevent repeated execution
        }
     ...
     * @static
     */
    $D.parseDate = function(input, format) {
        if ($D.parseFunctions[format] == null) {
            $D.createParser(format);
        }
        var func = $D.parseFunctions[format];
        return Date[func](input);
    };

    // private
    $D.createParser = function(format) {
        var funcName = "parse" + $D.parseFunctions.count++;
        var regexNum = $D.parseRegexes.length;
        var currentGroup = 1;
        $D.parseFunctions[format] = funcName;
        ...
        for (var i = 0; i < format.length; ++i) {
            ...
            if (!special && ch == "\\") {
                ...
                var obj = $D.formatCodeToRegex(ch, currentGroup);
                currentGroup += obj.g;
                regex += obj.s;
                if (obj.g && obj.c) {
                    code += obj.c;
        ...
        $D.parseRegexes[regexNum] = new RegExp("^" + regex + "$", "i");
        eval(code);
    };

    // private
    $D.formatCodeToRegex = function(character, currentGroup) {
        // Note: currentGroup - position in regex result array (see notes for Date.parseCodes above)
        var p = $D.parseCodes[character];

        if (p) {
          p = Ext.type(p) == 'function'? p() : p;
          $D.parseCodes[character] = p; // reassign function result to prevent repeated execution
        }
    ...
    var $f = $D.formatCodeToRegex; // private shorthand
    // private - Date.parseCodes
    $D.parseCodes = {
        /*
         * Notes:
        ...
        D: function() {
            for (var a = [], i = 0; i < 7; a.push($D.getShortDayName(i)), ++i); // get localised short day names
            return {
                g:0,
                c:null,
                s:"(?:" + a.join("|") +")"
        ...
        M: function() {
            for (var a = [], i = 0; i < 12; a.push($D.getShortMonthName(i)), ++i); // get localised short month names
            return Ext.applyIf({
                s:"(" + a.join("|") + ")"
            }, $f("F"));
        },
    ...
    $P.getDayOfYear = function() {
        var num = 0;
        $D.daysInMonth[1] = this.isLeapYear() ? 29 : 28;
     ...
     */
    $P.getWeekOfYear = function() {
        // adapted from http://www.merlyn.demon.co.uk/weekcalc.htm
        var ms1d = 864e5; // milliseconds in a day
        var ms7d = 7 * ms1d; // milliseconds in a week
        var DC3 = $D.UTC(this.getFullYear(), this.getMonth(), this.getDate() + 3) / ms1d; // an Absolute Day Number
        var AWN = Math.floor(DC3 / 7); // an Absolute Week Number
        var Wyr = new Date(AWN * ms7d).getUTCFullYear();
        return AWN - Math.floor($D.UTC(Wyr, 0, 7) / ms7d) + 1;
    };
     ...
     * @return {Number} The day number (0-6).
     */
    $P.getLastDayOfMonth = function() {
        var day = (this.getDay() + ($D.daysInMonth[this.getMonth()] - this.getDate())) % 7;
        return (day < 0) ? (day + 7) : day;
    };
     ...
     * @return {Date}
     */
    $P.getFirstDateOfMonth = function() {
    ...
    $P.getDaysInMonth = function() {
        $D.daysInMonth[1] = this.isLeapYear() ? 29 : 28;
        return $D.daysInMonth[this.getMonth()];
    };
    ...
    // private
    $D.daysInMonth = [31,28,31,30,31,30,31,31,30,31,30,31];
     ...
     * @static
     */
    $D.getShortMonthName = function(month) {
        return $D.monthNames[month].substring(0, 3);
    }
    ...
    $D.getShortDayName = function(day) {
        return $D.dayNames[day].substring(0, 3);
    }
     ...
     * @static
     */
    $D.getMonthNumber = function(name) {
        // handle camel casing for english month names (since the keys for the Date.monthNumbers hash are case sensitive)
        return $D.monthNumbers[name.substring(0, 1).toUpperCase() + name.substring(1, 3).toLowerCase()];
    }
    ...
    if(Ext.isSafari){
      $D.brokenSetMonth = $P.setMonth;
      $P.setMonth = function(num){
        if(num <= -1){
          var n = Math.ceil(-num);
          var back_year = Math.ceil(n/12);
          var month = (n % 12) ? 12 - n % 12 : 0 ;
          this.setFullYear(this.getFullYear() - back_year);
          return $D.brokenSetMonth.call(this, month);
        } else {
          return $D.brokenSetMonth.apply(this, arguments);
        }
      };
    }
