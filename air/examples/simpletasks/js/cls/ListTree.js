tx.tree.ListTree = function(config){
	tx.tree.ListTree.superclass.constructor.call(this, Ext.apply({
		id:'list-tree',
		animate:false,
		//rootVisible:false,
		region:'west',
...
Ext.extend(tx.tree.ListTree, Ext.tree.TreePanel, {
	...
	initComponent : function(){
		...
		this.loader = new tx.tree.ListLoader({
			store: tx.data.lists
		});
		tx.tree.ListTree.superclass.initComponent.call(this);
	...
	removeList : function(s){
		if (s && s.attributes.editable) {
			Ext.air.Msg.confirm('Confirm', 'Are you sure you want to delete "' + Ext.util.Format.htmlEncode(s.text) + '"?', function(btn){
				if (btn == 'yes') {
					if (s.nextSibling) {
