tx.grid.TaskGrid = function(){
	
	// custom columns
	var completeColumn = new tx.grid.CompleteColumn();
	var reminderColumn = new tx.grid.ReminderColumn();
	
	tx.grid.TaskGrid.superclass.constructor.call(this, {
		id:'tasks-grid',
        store: tx.data.tasks,
        ...
        columns: [
            ...
            },
            {
                ...
                sortable: true,
                dataIndex: 'listId',
                editor: new tx.form.ListSelector({
			        store:tx.data.lists
			    }),
				renderer : function(v){
...
Ext.extend(tx.grid.TaskGrid, Ext.grid.EditorGridPanel, {
...
tx.grid.TaskView = Ext.extend(Ext.grid.GroupingView, {
	forceFit:true,
    ignoreAdd: true,
    emptyText: 'There are no tasks to show in this list.',

	afterRenderUI: function() {
    	tx.grid.TaskView.superclass.afterRenderUI.call(this);
    	Ext.fly(this.innerHd).createChild({
    		cls: 'new-task-row',
    		children: [{
    			id: 'new-task-icon'
    		},{
...
tx.grid.TaskHeader = function(grid){
    ...
    var ntCat = new tx.form.ListSelector({
    ...
    function doBlur(){
        if(editing && !focused){
            ...
            if(!Ext.isEmpty(title)){
				tx.data.tasks.createTask(title, ntCat.getValue(), ntDue.getValue());
            	ntTitle.setValue('');
