...
Xp.ui.Playlist = Ext.extend(Ext.grid.GridPanel, {
	initComponent: function() {
		...
		this.bbar = [{
			text: 'Add',
			icon: '/resources/images/table_add.png',
			cls: 'x-btn-text-icon',
			handler: this.addBtnClick,
			scope: this
		},{
			text: 'Clear',
			icon: '/resources/images/table_lightning.png',
			cls: 'x-btn-text-icon',
			handler: this.clearBtnClick,
			scope: this
		},{
			text: 'Remove',
			icon: '/resources/images/table_delete.png',
			cls: 'x-btn-text-icon',
	...
	onRowDblClick: function(grid, rowIdx, e) {
		...
		var win = Ext.air.App.getRootHtmlWindow() || window;
		win = win.Ext.air.NativeWindowManager.get('main');
		win.play(r.get('url'));
	},
