...
Ext.ux.grid.filter.ListFilter = Ext.extend(Ext.ux.grid.filter.Filter, {
    /**
     * @cfg {Array} options If included will implicitly create a data
     * store using the supplied array of data.
     * Instead of specifying <code>options</code>, the <code>{@link #store}</code> config may be used.
     */
    /**
     * @cfg {Boolean} phpMode
     * Defaults to false.
...
filter[0][field] prod 
    ...
    phpMode : false,
    /**
     * @cfg {Ext.data.Store} store The {@link Ext.data.Store} this list should use as
     * its data source.
     * Instead of specifying a store, the <code>{@link #options}</code> config may be used.
     */
